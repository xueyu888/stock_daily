...
# 从数据库中获取创业板股票列表
sql = "SELECT DISTINCT ts_code FROM daily_price WHERE ts_code NOT LIKE '83%' AND ts_code NOT LIKE '688%'"
...
for ts_code in stocks:
    ...
    df = pd.DataFrame(engine.connect().execute(text(sql)), columns=[
                      'trade_date', 'ts_code', 'open_price', 'close_price', 'high', 'low', 'vol'])
        ...
        for i, row in df.iterrows():
            if status == 'start':
                if row['close_price'] > row['MA5']:
                    ...
                    gain = (temp_df['close_price'].iloc[5] -
                            temp_df['close_price'].iloc[0]) / temp_df['close_price'].iloc[0]
                    if gain > 0.20:  # 涨幅大于25%
                        status = 'rising'
                        last_rise = i
            elif status == 'rising':
                if row['close_price'] < row['MA5']:
                    if i+2 >= len(df):
                        break  # 数据不足三天，退出循环
                    next_three_days_df = df.loc[i:i+2]  # 取接下来的五天数据，包括首次跌破的那一天
                    # 如果接下来的五天收盘价都在五日线之下
                    if next_three_days_df['close_price'].lt(next_three_days_df['MA5']).all():
                        status = 'falling'
                ...
                if row['close_price'] > df.loc[i-1, 'MA5']:
                    ...
                    selected_stocks.append(
                        {'date': row['trade_date'], 'stock': ts_code})  # 添加买入信号到列表

                    # 遍历每个买入信号，计算收益率并添加到列表中
                    for signal in selected_stocks:
                        date_buy = signal['date']
                        stock_buy = signal['stock']
                        buy_row = df.loc[(df.ts_code == stock_buy) & (
                            df.trade_date == date_buy)]
                        ...
                        # 确保 date_sell_index 小于等于 df 的最大索引值
                        if date_sell_index <= df.index.max():
                            sell_row = df.iloc[date_sell_index]
                        else:
                            # 如果 date_sell_index 超出了 df 的最大索引值，则将 sell_row 设为 None 或其他值
                            continue
...
with open('returns.csv', mode='w', newline='') as csv_file:
    ...
    for item in returns:
        writer.writerow(
            {'date': item['date'], 'stock': item['stock'], 'return_rate': item['return_rate']})

# 计算平均收益率并打印结果
average_return_rate = np.mean(returns.return_rate)
